...
void snapshot_actions::setup(CLI::App& app) {
   ...
   from_json->add_option("--input-file,-i", opt->input_file, "Snapshot file (json) to convert. Writes to <input>.bin if --output-file not specified (tmp state dir used).")->required();
   from_json->add_option("--output-file,-o", opt->output_file, "The file to write the binary snapshot to (absolute or relative path). If not specified then output is to <input-file>.bin.");
   ...
   from_json->callback([err_guard]() {err_guard(&snapshot_actions::run_from_json);});
}
